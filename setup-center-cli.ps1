...
#   [6] ElasticSearch Management   (install/repair/status)
#   [7] System Setup               (Network Info / Change Hostname / Create User)
#   [8] Tailscale VPN              (Install / Login / Connect / Status / Remove)
#   [9] Blockchain Dev Toolkit     (Ganache CLI, Truffle, Geth & Node.js)
#   [10] Win 11 Setup & OOBE Bypass
#   [0] Exit
# =============================================================================
# Usage:
...
# â”€â”€â”€ Desktop shortcut helper â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function Install-AnyDeskDirectly {
    ...
    try {
        ...
        Invoke-WebRequest -Uri "https://download.anydesk.com/AnyDesk.exe" -OutFile $exe -UseBasicParsing -TimeoutSec 60 -ErrorAction Stop
        if (-not (Test-Path $exe) -or (Get-Item $exe).Length -eq 0) { throw "Downloaded file is missing or empty." }
...
function Install-UltraViewerDirectly {
    ...
    try {
        if (-not (Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force -ErrorAction Stop | Out-Null }
        Invoke-WebRequest -Uri "https://ultraviewer.net/UltraViewer_setup.exe" -OutFile $setup -UseBasicParsing -TimeoutSec 60 -ErrorAction Stop
        if (-not (Test-Path $setup) -or (Get-Item $setup).Length -eq 0) { throw "Downloaded file is missing or empty." }
...
function Install-MSOffice {
    ...
    try {
        ...
        if (-not (Test-Path $ZipPath) -or (Get-Item $ZipPath).Length -eq 0) { throw "Downloaded file is missing or empty." }
    ...
    try {
        ...
        Write-OK "Extracted."
    } catch { Write-ERR "Extraction failed: $($_.Exception.Message)"; Pause-Menu; return }
...
function Update-AllSoftware {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) { Write-OK "All software updated." }
        else { Write-ERR "winget exited with code $LASTEXITCODE - some packages may not have updated." }
    } catch { Write-ERR "$($_.Exception.Message)" }
    Pause-Menu
}
...
# =============================================================================
# [6] OFFICE SOFTWARE - ElasticSearch
# =============================================================================
...
function Install-ElasticSearch {
    Show-Header; Write-Host "  [6.1] INSTALL ELASTICSEARCH 8.11.1" -ForegroundColor Cyan; Write-Host ""
    ...
    try {
        if (Test-Path "$dir\bin\elasticsearch-service.bat") { Write-OK "Elasticsearch already installed." }
            ...
            if (-not (Test-Path $root)) { New-Item $root -ItemType Directory -Force -ErrorAction Stop | Out-Null }
            $local = "$env:TEMP\$zip"; $nasZ = "$nas\$zip"; $ok=$false
            if (Test-Path $nasZ) { try { Copy-Item $nasZ $local -Force -ErrorAction Stop; $ok=$true } catch { Write-WARN "NAS copy failed, will download instead." } }
            if (-not $ok) {
                Write-INFO "Downloading (may take several minutes)..."
                Invoke-WebRequest -Uri $url -OutFile $local -UseBasicParsing -TimeoutSec 300 -ErrorAction Stop
            }
            if (-not (Test-Path $local) -or (Get-Item $local).Length -eq 0) { throw "Downloaded file '$local' is missing or empty." }
            Write-INFO "Extracting..."; Expand-Archive $local $root -Force -ErrorAction Stop
            $ex="$root\elasticsearch-$ver"; if(Test-Path $ex){Rename-Item $ex $ver -ErrorAction Stop}
        }
        if (-not (Test-Path "$java\bin\java.exe")) {
            $jdkLocal="$env:TEMP\jdk-17-installer.exe"
            if (Test-Path $jdkNas) { Copy-Item $jdkNas $jdkLocal -Force -ErrorAction Stop }
            if (Test-Path $jdkLocal) { Write-WARN "Complete JDK installer..."; Start-Process $jdkLocal -Wait }
            else { Write-WARN "JDK 17 not found on NAS and no other source configured - install it manually if the service fails to start." }
        }
        ...
        New-Item "$data\config" -ItemType Directory -Force -ErrorAction Stop | Out-Null
        New-Item "$data\data"   -ItemType Directory -Force -ErrorAction Stop | Out-Null
        New-Item "$data\logs"   -ItemType Directory -Force -ErrorAction Stop | Out-Null
        $src="$dir\config"; if(Test-Path "$src\elasticsearch.yml"){Copy-Item "$src\*" "$data\config" -Recurse -Force -ErrorAction Stop}
        "bootstrap.memory_lock: false`ncluster.name : elasticsearch`nhttp.port: 9200`nnode.name : $env:COMPUTERNAME`npath.data: $data\data`npath.logs: $data\logs`npath.repo: $data\backup`ntransport.port: 9300`nxpack.license.self_generated.type: basic`nxpack.security.enabled: true`naction.auto_create_index: .monitoring*,.watches,.triggered_watches,.watcher-history*,.ml*" | Set-Content "$data\config\elasticsearch.yml" -ErrorAction Stop
...
function Repair-ElasticSearch {
    Show-Header; Write-Host "  [6.2] REPAIR ELASTICSEARCH" -ForegroundColor Yellow; Write-Host ""
...
function Get-ElasticSearchStatus {
    Show-Header; Write-Host "  [6.3] ELASTICSEARCH STATUS" -ForegroundColor Cyan; Write-Host ""
...
function Show-OfficeSoftwareMenu {
    while ($true) {
        Show-Header
        Write-Host "  [6] ELASTICSEARCH MANAGEMENT" -ForegroundColor Yellow; Write-Host ""
        Write-Host "  [1] Install ElasticSearch"                         -ForegroundColor White
        Write-Host "  [2] Repair ElasticSearch  (reset admin pw + restart)" -ForegroundColor Yellow
        ...
        switch ($sub) {
            ...
            '2' { Repair-ElasticSearch }
            '3' { Get-ElasticSearchStatus }
            '0' { return }
...
# =============================================================================
# [7] SYSTEM SETUP - Network Info / Hostname / Create User
# =============================================================================
function Get-NetworkInfo {
    Show-Header; Write-Host "  [7.1] NETWORK INFORMATION" -ForegroundColor Cyan; Write-Host ""
...
function Set-PCHostname {
    Show-Header; Write-Host "  [7.2] CHANGE PC HOSTNAME" -ForegroundColor Cyan; Write-Host ""
...
function New-WorkUser {
    Show-Header; Write-Host "  [7.3] CREATE WORK USER" -ForegroundColor Cyan; Write-Host ""
...
function Show-SystemSetupMenu {
    while ($true) {
        Show-Header
        Write-Host "  [7] SYSTEM SETUP" -ForegroundColor Yellow; Write-Host ""
...
function Install-Tailscale {
    Show-Header; Write-Host "  [8.1] INSTALL TAILSCALE" -ForegroundColor Cyan; Write-Host ""
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) { Write-OK "Tailscale installed. PATH will be available after restarting the shell." }
        else { Write-ERR "winget exited with code $LASTEXITCODE - Tailscale may not be installed." }
    } catch { Write-ERR "Install failed: $($_.Exception.Message)" }
    Pause-Menu
}

function Invoke-TailscaleLogin {
    Show-Header; Write-Host "  [8.2] TAILSCALE LOGIN / REGISTER" -ForegroundColor Cyan; Write-Host ""
...
function Invoke-TailscaleConnect {
    Show-Header; Write-Host "  [8.3] TAILSCALE CONNECT (Accept Routes)" -ForegroundColor Cyan; Write-Host ""
...
function Invoke-TailscaleFullReset {
    Show-Header; Write-Host "  [8.4] TAILSCALE FULL RESET + CONNECT" -ForegroundColor Cyan; Write-Host ""
...
function Invoke-TailscaleExitNode {
    Show-Header; Write-Host "  [8.5] TAILSCALE CONNECT WITH EXIT NODE" -ForegroundColor Cyan; Write-Host ""
...
function Get-TailscaleStatus {
    Show-Header; Write-Host "  [8.6] TAILSCALE STATUS / DIAGNOSTICS" -ForegroundColor Cyan; Write-Host ""
    try {
...
function Uninstall-Tailscale {
    Show-Header; Write-Host "  [8.7] UNINSTALL TAILSCALE" -ForegroundColor Red; Write-Host ""
    $c = Read-Host "  Confirm uninstall Tailscale? (y/N)"
    if ($c -notmatch '^[Yy]$') { Pause-Menu; return }
    try {
        tailscale logout 2>$null
        winget uninstall --id Tailscale.Tailscale --silent
        if ($LASTEXITCODE -eq 0) { Write-OK "Tailscale uninstalled." }
        else { Write-ERR "winget exited with code $LASTEXITCODE - Tailscale may not be fully removed." }
...
function Show-TailscaleMenu {
    while ($true) {
        Show-Header
        Write-Host "  [8] TAILSCALE VPN" -ForegroundColor Yellow
...
function Show-BlockchainMenu {
    while ($true) {
        Show-Header
        Write-Host "  [9] BLOCKCHAIN & ETHEREUM DEVELOPER TOOLKIT" -ForegroundColor Yellow; Write-Host ""
...
function Show-Win11BypassMenu {
    Show-Header
    Write-Host "  [10] WINDOWS 11 SETUP & OOBE BYPASS HELPERS" -ForegroundColor Yellow; Write-Host ""
...
while ($true) {
    ...
    @(
        ...
        @{K="6";L="ElasticSearch Management";   D="Install / repair / status"},
        @{K="7";L="System Setup";              D="Network info / Hostname / Create user"},
        @{K="8";L="Tailscale VPN";             D="Install / Login / Connect / Status / Remove"},
        @{K="9";L="Blockchain Dev Toolkit";    D="Ganache CLI, Truffle, Geth & Node.js"},
        @{K="10";L="Win 11 Setup & OOBE Bypass";D="BypassNRO & Microsoft Account bypass tips"},
        @{K="0";L="Exit";                       D=""}
    ) | ForEach-Object {
    ...
    $opt = (Read-Host "`n  Enter choice [0-10]").Trim()
    switch ($opt) {
        ...
        '6' { Show-OfficeSoftwareMenu }
        '7' { Show-SystemSetupMenu }
        '8' { Show-TailscaleMenu }
        '9' { Show-BlockchainMenu }
        '10' { Show-Win11BypassMenu }
        '0' { Write-Host "`n  Goodbye!`n" -ForegroundColor Cyan; exit 0 }
        default { Write-WARN "Invalid choice - enter 0-10."; Start-Sleep -Seconds 1 }
    }
}
